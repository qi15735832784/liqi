## ***\*20、简述Kubernetes创建一个Pod的主要流程？\****263
...
## ***\*23、简述Kubernetes Pod的\****[***\*LivenessProbe探针\****](https://www.zhihu.com/search?q=LivenessProbe探针&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={:,:2200949037})***\*的常见方式\****152
...
## ***\*24、简述Kubernetes Pod的常见调度方式\****243
...
## ***\*26、简述Kubernetes deployment升级过程\****132
...
## ***\*33、简述Kubernetes外部如何访问集群内的服务\****254
...
同时当Ingress Controller提供的是对外服务，则实际上实现的是[边缘路由器](https://www.zhihu.com/search?q=边缘路由器&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={:,:2200949037})的功能。

## 38、简述Kubernetes Scheduler作用及实现原理320
...
Kubernetes Scheduler的作用是将待调度的Pod（API新创建的Pod、Controller Manager为补足副本而创建的Pod等）按照特定的[调度算法](https://www.zhihu.com/search?q=调度算法&search_source=Entity&hybrid_search_source=Entity&hybrid_search_extra={:,:2200949037})和调度策略绑定（Binding）到集群中某个合适的Node上，并将绑定信息写入etcd中。
